...
func generateDepartments() error {
	...
	for _, department := range departments {
		err := DB.FirstOrCreate(&Department{}, department).Error
		if err != nil {
			return err
		}
		//添加上级机构id
		...
		if department.Name == "北京公司" { //如果是北京公司，就不做处理
			...
			//查找上级部门的信息
			err = DB.Where("name = ?", "水泥工程事业部").First(&superiorDepartment).Error
			if err != nil {
				return err
			}
			//把上级部门的id赋值给本部门
			err = DB.Model(&Department{}).Where("name = ?", department.Name).Update("superior_id", superiorDepartment.ID).Error
			if err != nil {
				return err
			}
		} else { //如果非水泥工程事业部的其他部门
			//查找上级部门的信息
			err = DB.Where("name = ?", "北京公司").First(&superiorDepartment).Error
			if err != nil {
				return err
			}
			//把上级部门的id赋值给本部门
